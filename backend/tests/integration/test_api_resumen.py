...
def test_api_resumen_con_ingresos_y_gastos(client: TestClient):
    ...
    response = client.get("/api/resumen", params={"mes": "2026-08"})
...
def test_api_resumen_aislado_por_usuario(client: TestClient, client_usuario_2: TestClient):
    """
    3. El resumen solo agrega los movimientos del usuario autenticado.

    Sustituye a la antigua prueba de "usuario inexistente": ya no es posible
    pedir el resumen de otra cuenta, porque el usuario procede del token.
    """
    _crear_movimiento(client, "ingreso", "1000000.00", "2026-08-05")

    data_u2 = client_usuario_2.get("/api/resumen", params={"mes": "2026-08"}).json()
    assert data_u2["id_usuario"] == 2
    assert data_u2["total_ingresos"] == "0.00"
    assert data_u2["total_gastos"] == "0.00"
    assert data_u2["balance"] == "0.00"

    data_u1 = client.get("/api/resumen", params={"mes": "2026-08"}).json()
    assert data_u1["id_usuario"] == 1
    assert data_u1["total_ingresos"] == "1000000.00"
...
def test_api_resumen_mes_formato_invalido(client: TestClient, mes_invalido):
    ...
    response = client.get("/api/resumen", params={"mes": mes_invalido})
...
def test_api_resumen_mes_inexistente(client: TestClient, mes_inexistente):
    ...
    response = client.get("/api/resumen", params={"mes": mes_inexistente})
    assert response.status_code == 400
...
def test_api_resumen_ignora_id_usuario_de_la_query(client: TestClient):
    """
    Un `id_usuario` en la query string se ignora por completo.

    Antes era un parámetro de entrada; ahora la identidad sale del token, así
    que enviarlo no debe permitir consultar el resumen de otra cuenta.
    """
